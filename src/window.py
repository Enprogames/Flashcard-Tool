...
class ItemSelectionFrame(tk.Frame):
    ...
    def __init__(self, parent, items=[], start_command=None, width=600, height=600):
        ...
        self.item_selection_scrollbar = tk.Scrollbar(self.scrollable_item_selection, orient='vertical', command=self.scrollable_canvas.yview, bg='#263238')
        ...
        self.autoflip_interval_box = tk.Entry(self.autoflip_entry_frame, width=5, foreground='white', bg='#263238', font=('consolas', 15, 'normal'), textvariable=self.autoflip_interval)
        ...
        self.start_button = tk.Button(self.options_frame, text='START', foreground='white', background='grey25', command=start_command, font=('consolas', 15, 'bold'))
...
class FlashcardTermFrame(tk.Frame):
    ...
    def __init__(self, parent, text, flip_command=None, width=600, height=400, definition_first=False):
        ...
        self.text = text
        self.text_label = tk.Label(self, text=self.text, foreground='white', background='#263238', font=('consolas', resize_font(len(text), 500), 'normal' if definition_first else 'bold'), wraplength=400, justify=tk.CENTER)
...
class FlashcardDefinitionFrame(tk.Frame):
    ...
    def __init__(self, parent, text, next_command=None, width=600, height=400, definition_first=False):
        ...
        self.text = text
        self.text_label = tk.Label(self, text=self.text, foreground='white', background='#263238', font=('consolas', resize_font(len(text), 500), 'bold' if definition_first else 'normal'), wraplength=400, justify=tk.CENTER)
        self.text_label.place(relx=0.5, rely=0.5, anchor="center")
...
class FlashcardFrame(tk.Frame):
    ...
    def __init__(self, parent, term, definition = "", definition_first=False, next_command=None, quit_command=None, width=600, height=400):
        ...
        self.term_frame = FlashcardTermFrame(self, text=term, flip_command=self.flip_card, width=self.width, height=self.height, definition_first=definition_first)
        self.definition_frame = FlashcardDefinitionFrame(self, text=definition, next_command=next_command, width=self.width, height=self.height, definition_first=definition_first)
